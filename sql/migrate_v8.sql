...
values
  ...
  ('board_cols', '{"value":24}'),
  ('board_rows', '{"value":14}'),
  ('roll_sync_delay_ms', '{"value":1800}'),
  ('dice_animation_ms', '{"value":1450}'),
...
update public.game_cards set card_group = 'chance' where card_group is null or btrim(card_group) = '';
update public.game_tiles set card_group = 'chance' where kind = 'card' and (card_group is null or btrim(card_group) = '');
update public.game_tiles set card_group = null where kind <> 'card' and btrim(coalesce(card_group, '')) = '';


-- v8 board path editor
alter table public.game_tiles add column if not exists board_x integer;
alter table public.game_tiles add column if not exists board_y integer;
alter table public.game_tiles add column if not exists next_sort_order integer;

with ordered as (
  select id, sort_order, lead(sort_order) over (order by sort_order asc, id asc) as next_sort, first_value(sort_order) over (order by sort_order asc, id asc) as first_sort
  from public.game_tiles
)
update public.game_tiles t
set next_sort_order = coalesce(t.next_sort_order, ordered.next_sort, ordered.first_sort)
from ordered
